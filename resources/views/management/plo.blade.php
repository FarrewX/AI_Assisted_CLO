...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body data-current-year="{{ $selectedYear ?? '' }}" class="text-slate-grey font-sans antialiased min-h-screen">
        ...
        <div class="container mx-auto p-4 pt-24 max-w-6xl relative">
            ...
            <div class="mb-8 bg-pure-white p-6 rounded-2xl shadow-sm border border-cloud-blue flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-deep-navy tracking-tight">จัดการข้อมูล PLOs</h1>
                        <p class="text-sm text-slate-grey mt-1">Program Learning Outcomes</p>
                    </div>
                </div>
            ...
            @if(isset($selectedYear) && $selectedYear)
                ...
                <div class="flex justify-between items-center mb-4 px-2">
                    <h2 class="text-lg font-bold text-deep-navy">รายการ PLOs ปี {{ $selectedYear }}</h2>
                    ...
                    <div class="overflow-x-auto no-scrollbar">
                        <table class="min-w-full divide-y divide-cloud-blue/50">
                            <thead class="bg-cloud-blue/30">
                                <tr>
                                    <th class="px-6 py-4 text-center text-xs font-bold text-deep-navy uppercase tracking-wider w-20">PLO</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-deep-navy uppercase tracking-wider">คำอธิบาย (Description)</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-deep-navy uppercase tracking-wider w-32">Domain</th>
                                    <th class="px-6 py-4 text-left text-xs font-bold text-deep-navy uppercase tracking-wider w-32">Level</th>
                                    <th class="px-6 py-4 text-center text-xs font-bold text-deep-navy uppercase tracking-wider w-32">ประเภท</th>
                            ...
                            <tbody class="bg-pure-white divide-y divide-cloud-blue/50">
                                ...
                                <tr data-id="{{ $plo->id }}" 
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-slate-grey whitespace-pre-wrap leading-relaxed">{{ $plo->description }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-deep-navy font-medium">
                                        {{ $plo->domain ?: '-' }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-deep-navy font-medium">
                                        {{ $plo->learning_level ?: '-' }}
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if($plo->specific_lo)
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-cloud-blue/40 text-royal-blue border border-cloud-blue">Specific LO</span>
                                        @else
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-cloud-blue/20 text-slate-grey border border-cloud-blue">Generic LO</span>
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center text-slate-grey">
                                        <div class="flex flex-col items-center justify-center">
                                            <svg class="w-12 h-12 mb-3 text-cloud-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path></svg>
            ...
            @else
                <div class="bg-pure-white border-2 border-cloud-blue border-dashed rounded-3xl p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
                    ...
                    <p class="text-slate-grey max-w-md mx-auto">กรุณาเลือกปีการศึกษาจากเมนูด้านบน เพื่อดึงข้อมูล PLOs ขึ้นมาจัดการ</p>
                </div>
            @endif
        </div>
        ...
        @if(isset($selectedYear))
            ...
            <div id="add-plo-modal" class="fixed inset-0 flex items-center justify-center bg-deep-navy/70 z-50 hidden backdrop-blur-sm transition-opacity opacity-0">
                <div class="bg-pure-white rounded-2xl shadow-2xl p-8 min-w-[400px] max-w-lg w-full transform scale-95 transition-transform duration-300 border border-cloud-blue">
                    <h2 class="text-2xl font-bold mb-6 text-deep-navy border-b border-cloud-blue pb-4">เพิ่ม PLO ใหม่</h2>
                    
                    <div class="space-y-5">
                        <div>
                            <label class="block mb-1.5 text-sm font-bold text-deep-navy">PLO No.</label>
                            <select id="new-plo" class="border border-cloud-blue px-4 py-2.5 w-full rounded-xl bg-cloud-blue/10 focus:bg-pure-white focus:ring-4 focus:ring-royal-blue/20 focus:border-royal-blue outline-none transition-colors text-deep-navy">
                                ...
                                <optgroup label="ลำดับถัดไป">
                                    <option value="{{ $nextPloNumber }}" selected class="text-royal-blue font-bold">PLO {{ $nextPloNumber }} (ใหม่)</option>
                                </optgroup>
                            </select>
                        </div>
                        
                        <div>
                            <label class="block mb-1.5 text-sm font-bold text-deep-navy">คำอธิบาย (Description) <span class="text-red-500">*</span></label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block mb-1.5 text-sm font-bold text-deep-navy">Domain (เลือกได้สูงสุด 2)</label>
                                <select id="new-domain1" class="border border-cloud-blue px-4 py-2.5 w-full rounded-xl bg-cloud-blue/10 focus:bg-pure-white focus:ring-4 focus:ring-royal-blue/20 focus:border-royal-blue outline-none transition-colors cursor-pointer mb-2 text-deep-navy">
                                ...
                                <select id="new-domain2" class="border border-cloud-blue px-4 py-2.5 w-full rounded-xl bg-cloud-blue/10 focus:bg-pure-white focus:ring-4 focus:ring-royal-blue/20 focus:border-royal-blue outline-none transition-colors cursor-pointer text-deep-navy">
                            ...
                            <div>
                                <label class="block mb-1.5 text-sm font-bold text-deep-navy">Level</label>
                                <select id="new-level" disabled class="border border-cloud-blue px-4 py-2.5 w-full rounded-xl bg-cloud-blue/30 text-slate-grey focus:bg-pure-white focus:ring-2 focus:ring-royal-blue outline-none transition-colors cursor-not-allowed">
                                    <option value="">-- เลือก Domain ก่อน --</option>
                                </select>
                            </div>
                        </div>

                        <div class="pt-2">
                            <label class="block mb-2 text-sm font-bold text-deep-navy">ประเภทของ LO</label>
                            <div class="flex gap-4 p-1">
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <input type="radio" name="new_lo_type" value="1" class="w-5 h-5 text-royal-blue focus:ring-royal-blue border-cloud-blue cursor-pointer" checked>
                                    <span class="text-deep-navy group-hover:text-royal-blue font-medium transition-colors">Specific LO</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <input type="radio" name="new_lo_type" value="0" class="w-5 h-5 text-royal-blue focus:ring-royal-blue border-cloud-blue cursor-pointer">
                                    <span class="text-deep-navy group-hover:text-royal-blue font-medium transition-colors">Generic LO</span>
                                </label>
                            </div>
                        </div>
                    </div>

                    <div class="flex justify-end gap-3 pt-6 mt-4 border-t border-cloud-blue">
                        <button id="add-plo-cancel" class="px-5 py-2.5 bg-cloud-blue/30 hover:bg-cloud-blue text-deep-navy font-medium rounded-xl transition-colors">ยกเลิก</button>
                        <button id="add-plo-save" class="px-5 py-2.5 bg-royal-blue hover:bg-deep-navy text-pure-white font-medium rounded-xl shadow-md transition-colors flex items-center gap-2">
                            บันทึกข้อมูล
                        </button>
                    </div>
                </div>
            ...
            <div id="edit-plo-modal" class="fixed inset-0 flex items-center justify-center bg-deep-navy/70 z-50 hidden backdrop-blur-sm transition-opacity opacity-0">
                <div class="bg-pure-white rounded-2xl shadow-2xl p-8 min-w-[400px] max-w-lg w-full transform scale-95 transition-transform duration-300 border border-cloud-blue">
                    <h2 class="text-2xl font-bold mb-6 text-deep-navy border-b border-cloud-blue pb-4">แก้ไข PLO</h2>
                    <input type="hidden" id="edit-id">

                    <div class="space-y-5">
                        <div>
                            <label class="block mb-1.5 text-sm font-bold text-deep-navy">PLO No.</label>
                            <input id="edit-plo-num" type="number" class="border border-cloud-blue px-4 py-2.5 w-full rounded-xl bg-cloud-blue/30 text-slate-grey cursor-not-allowed font-bold" readonly>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="edit-domain1" class="border border-cloud-blue px-4 py-2.5 w-full rounded-xl bg-pure-white focus:ring-4 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-colors cursor-pointer mb-2 text-deep-navy">
                                    <option value="" hidden>-- เลือก Domain ที่ 1 --</option>
                                    <option value="Knowledge">Knowledge</option>
                                    <option value="Application and Responsibility">Application and Responsibility</option>
                                    <option value="Skill">Skill</option>
                                ...
                                <select id="edit-domain2" class="border border-cloud-blue px-4 py-2.5 w-full rounded-xl bg-pure-white focus:ring-4 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-colors cursor-pointer text-deep-navy">
                                    <option value="">-- ไม่ระบุ (Domain ที่ 2) --</option>
                                    <option value="Knowledge">Knowledge</option>
                                    <option value="Application and Responsibility">Application and Responsibility</option>
                                    <option value="Skill">Skill</option>
                            ...
                            <div>
                                <label class="block mb-1.5 text-sm font-bold text-deep-navy">Level</label>
                                <select id="edit-level" disabled class="border border-cloud-blue px-4 py-2.5 w-full rounded-xl bg-cloud-blue/30 text-slate-grey focus:ring-2 focus:ring-amber-500 outline-none transition-colors cursor-not-allowed">
                                    <option value="">-- เลือก Domain ก่อน --</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="pt-2">
                            ...
                            <div class="flex gap-4 p-1">
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <input type="radio" name="edit_lo_type" id="edit-specific-lo" value="1" class="w-5 h-5 text-amber-500 focus:ring-amber-500 border-cloud-blue cursor-pointer">
                                    <span class="text-deep-navy group-hover:text-amber-600 font-medium transition-colors">Specific LO</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer group">
                                    <input type="radio" name="edit_lo_type" id="edit-generic-lo" value="0" class="w-5 h-5 text-amber-500 focus:ring-amber-500 border-cloud-blue cursor-pointer">
                                    <span class="text-deep-navy group-hover:text-amber-600 font-medium transition-colors">Generic LO</span>
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-6 mt-4 border-t border-cloud-blue">
                        <button id="edit-plo-cancel" class="px-5 py-2.5 bg-cloud-blue/30 hover:bg-cloud-blue text-deep-navy font-medium rounded-xl transition-colors">ยกเลิก</button>
                        <button id="edit-plo-save" class="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-pure-white font-medium rounded-xl shadow-md transition-colors flex items-center gap-2">
                            บันทึกการแก้ไข
                        </button>
                    </div>
                </div>
            ...
            <div class="relative w-full max-w-sm p-6 mx-4 bg-pure-white shadow-2xl rounded-3xl transform transition-all duration-300 scale-95 border border-cloud-blue" id="confirm-content">
                <div class="flex flex-col items-center text-center">
                    ...
                    <div class="mb-4 p-3 bg-cloud-blue/20 rounded-xl border border-cloud-blue w-full">
                        ...
                        <p id="confirm-plo-num" class="text-center font-extrabold text-red-600 text-2xl">-</p>
                    </div>
