...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'AI-Assisted CLO') }} | จัดการ LLLs</title>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                ...
                                <tr data-id="{{ $lll->id }}" 
                                    ...
                                    <td class="px-6 py-4 text-center font-black text-baltic-blue text-lg">
                                        LLL {{ $lll->num_LLL }}
                                    </td>
                                        ...
                                        @if(!empty($lll->check_LLL))
                                            <div class="flex flex-wrap justify-center gap-1.5">
                                                @foreach(explode(',', $lll->check_LLL) as $plo_num)
                                                    <span class="inline-flex items-center px-2 py-1 rounded-md text-xs font-bold bg-blue-100 text-baltic-blue border border-blue-200">
                                                        PLO {{ trim($plo_num) }}
                                                    </span>
                                                @endforeach
                                            </div>
            ...
            <div id="add-lll-modal" class="fixed inset-0 flex items-center justify-center bg-gray-900/60 z-50 hidden backdrop-blur-sm transition-opacity opacity-0">
                <div class="bg-white rounded-2xl shadow-2xl p-8 min-w-[400px] max-w-lg w-full transform scale-95 transition-transform duration-300">
                    ...
                    <div class="flex justify-end gap-3 pt-8 mt-2">
                        ...
                        <button id="add-lll-save" class="px-5 py-2.5 bg-[#035AA6] hover:bg-[#6CBAD9] text-white font-medium rounded-xl shadow-md transition-colors flex items-center gap-2">บันทึกข้อมูล</button>
                    </div>
                </div>
            </div>
            ...
            <div id="edit-lll-modal" class="fixed inset-0 flex items-center justify-center bg-gray-900/60 z-50 hidden backdrop-blur-sm transition-opacity opacity-0">
                <div class="bg-white rounded-2xl shadow-2xl p-8 min-w-[400px] max-w-lg w-full transform scale-95 transition-transform duration-300">
                    ...
                    <div class="flex justify-end gap-3 pt-8 mt-2">
                        ...
                        <button id="edit-lll-save" class="px-5 py-2.5 bg-[#035AA6] hover:bg-[#6CBAD9] text-white font-medium rounded-xl shadow-md transition-colors flex items-center gap-2">บันทึกการแก้ไข</button>
                    </div>
                </div>
            </div>
        @endif
